...
export function BuildTableForData(sectID, tableID, data, pAddRowName, pAddDisplayCol, pColumns) {
    var addRowName = pAddRowName ?? true;
    var addDisplayCol = pAddDisplayCol ?? false;
    var columns = pColumns ?? new Array();
    if (addDisplayCol) {
...
export function BuildBasicTable(sect, data, paddRowName, prebuild, paddDisplayCol) {
...
// ===========================================
function aTable(className, idName) {
    return anElement("table", className, idName);
}
function aTableRow(className, idName) {
    return anElement("tr", className, idName);
}
function aTableHeaderCell(className, idName) {
    return anElement("th", className, idName);
}
function aTableData(className, idName) {
    return anElement("td", className, idName);
}
function anElement(tagName, className, idName) {
    var ret = document.createElement(tagName);
    if (className != undefined)
        ret.setAttribute("class", className);
    if (idName != undefined)
        ret.setAttribute("id", idName);
    return ret;
}
...
export class TrendData {
    constructor(numberOfPoints) {
        ...
        for (var ii = 0; ii < this.maxDataPoints; ii++)
            this.dataPoints[ii] = 0;
    }
    AddPoint(pnt) {
